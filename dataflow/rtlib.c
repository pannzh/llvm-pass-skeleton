...
void __loadcheck(unsigned char *ptr, size_t size, size_t line) {
    dfsan_label temp = dfsan_read_label(ptr, size);
    int tainted = initialized ? dfsan_has_label(temp, src_label) : 0;
    printf("[%d] runtime load check: %p:%zu in line %zu\n",
            tainted, ptr, size, line);
    return;
}
void __storecheck(unsigned char *ptr, size_t size, size_t line) {
    dfsan_label temp = dfsan_read_label(ptr, size);
    int tainted = initialized ? dfsan_has_label(temp, src_label) : 0;
    printf("[%d] runtime store check: %p:%zu in line %zu\n",
            tainted, ptr, size, line);
    return;
}
